...
(function (api, $) {
  api.controlConstructor["bloglo-repeater"] = api.Control.extend({
    ready: function () {
      var control = this;
      setTimeout(function () {
        control._init();
    ...
    _init: function () {
      ...
      if (!window._upload_bloglo) {
        ...
        window._upload_bloglo = wp.media({
      ...
      window._upload_bloglo.on("close", function () {
        // get selections and save to hidden input plus other AJAX stuff etc.
        var selection = window._upload_bloglo.state().get("selection");

        // console.log(selection);
      });
      window.media_current = {};
      window.media_btn = {};
      window._upload_bloglo.on("select", function () {
        // Grab our attachment selection and construct a JSON representation of the model.
        var media_attachment = window._upload_bloglo
          .state()
          .get("selection")
          .first()
          .toJSON();
      ...
      control.handleGradient = function ($context) {
        ...
        $(".item-gradient", $context).each(function () {
          ...
          _item.find(".bloglo-range-wrapper").each(function () {
            var $this = $(this);
          ...
          _item.on("change", ".bloglo-select-wrapper", function () {
            control.update_value();
          });
        });
      };
      ...
      control.handleBackground = function ($context) {
        ...
        $(".item-design-options", $context).each(function () {
          ...
          // Linked button
          _item.on("click", ".bloglo-spacing-linked", function () {
            // Set up variables
            var $this = $(this);
          ...
          // Unlinked button
          _item.on("click", ".bloglo-spacing-unlinked", function () {
            // Set up variables
            var $this = $(this);
					...
					$.each(
						...
						function ( fieldId, field ) {
							if ( field.type === 'select' && field.is_select2 ) {
								...
								if ( $select.length ) {
									var select2Config = {
										placeholder: field.placeholder || ( typeof bloglo_customizer_localized !== 'undefined' ? bloglo_customizer_localized.strings.select_category : 'Select' ),
										allowClear: true,
									...
									if ( field.data_source ) {
										select2Config.ajax = {
											...
											error: function(error) {
												console.error( typeof bloglo_customizer_localized !== 'undefined' ? bloglo_customizer_localized.strings.error_loading_data : 'Error loading data', error );
											}
										};
									}
      ...
      control.actions = function ($context) {
        if (control.params.live_title_id) {
          ...
          if ("bloglo_section_order_styling" === control.id) {
...
jQuery(document).ready(function ($) {
  ...
  var options_font_type = "",
    icon_group = "";
  $.each(Bloglo_Icon_Picker.fonts, function (key, font) {
    font = $.extend(
      {},
      {
        url: "",
  ...
  icon_picker
    ...
    .attr("placeholder", Bloglo_Icon_Picker.search);
